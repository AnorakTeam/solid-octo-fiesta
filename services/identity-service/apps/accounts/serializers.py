...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated):
        user = User.objects.create_user(
            validated['email'],
            validated['password'],
            nickname=generate_nickname(validated['email'])
        )
        # Publicar evento desacoplado para inicialización en otros microservicios
...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    def validate_nickname(self, value):
        ...
        if not re.fullmatch(r'[a-z0-9_]{3,40}', nickname):
            raise serializers.ValidationError(
                'Usa entre 3 y 40 caracteres: letras, números o guion bajo.')
    ...
    def validate_profile_icon(self, value):
        ...
        if value.size > 2 * 1024 * 1024:
            raise serializers.ValidationError(
                'El ícono no puede superar 2 MB.')
